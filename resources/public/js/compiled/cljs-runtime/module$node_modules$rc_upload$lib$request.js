...
Object.keys(option.data).forEach(function(key){var value=option.data[key];Array.isArray(value)?value.forEach(function(item){formData.append("".concat(key,"[]"),item)}):formData.append(key,value)});option.file instanceof Blob?formData.append(option.filename,option.file,option.file.name):formData.append(option.filename,option.file);xhr.onerror=function(e){option.onError(e)};xhr.onload=function(){if(200>xhr.status||300<=xhr.status){var JSCompiler_temp_const=option.onError,msg="cannot ".concat(option.method,
