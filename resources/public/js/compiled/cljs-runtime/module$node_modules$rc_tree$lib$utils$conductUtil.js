...
(partialChecked=!0)});allChecked||checkedKeys.delete(parent.key);partialChecked&&halfCheckedKeys.add(parent.key)}visitedKeys.add(parent.key)}});return{checkedKeys:Array.from(checkedKeys),halfCheckedKeys:Array.from(removeFromCheckedKeys(halfCheckedKeys,checkedKeys))}}global=require("module$node_modules$$babel$runtime$helpers$interopRequireDefault");Object.defineProperty(exports,"__esModule",{value:!0});exports.conductCheck=function(keyList,checked,keyEntities,getCheckDisabled){var warningMissKeys=
[];getCheckDisabled=getCheckDisabled?getCheckDisabled:isCheckDisabled;keyList=new Set(keyList.filter(function(key){var hasEntity=!!(0,_keyUtil.default)(keyEntities,key);hasEntity||warningMissKeys.push(key);return hasEntity}));var levelEntities=new Map,maxLevel=0;Object.keys(keyEntities).forEach(function(key){key=keyEntities[key];var level=key.level,levelSet=levelEntities.get(level);levelSet||(levelSet=new Set,levelEntities.set(level,levelSet));levelSet.add(key);maxLevel=Math.max(maxLevel,level)});
(0,_warning.default)(!warningMissKeys.length,"Tree missing follow keys: ".concat(warningMissKeys.slice(0,100).map(function(key){return"'".concat(key,"'")}).join(", ")));return!0===checked?fillConductCheck(keyList,levelEntities,maxLevel,getCheckDisabled):cleanConductCheck(keyList,checked.halfCheckedKeys,levelEntities,maxLevel,getCheckDisabled)};exports.isCheckDisabled=isCheckDisabled;var _warning=global(require("module$node_modules$rc_util$lib$warning")),_keyUtil=global(require("module$node_modules$rc_tree$lib$utils$keyUtil"))}
